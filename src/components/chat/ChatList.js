...
export default function ChatList({ chats, onRefresh }) {
    ...
    const onDelete = async (data) => {
        ...
        try {
            ...
            // const response = await fetch(`http://localhost:3001/proxy?targetUrl=${encodeURIComponent(targetUrl)}`, {
            //     method: "POST",
            //     headers: {
            //         "Content-Type": "application/x-www-form-urlencoded", // HARUS pakai ini
            //     },
            //     body: formData.toString(), // <-- kirim dalam bentuk string, bukan object
            // });
        ...
        <div className="max-w-md mx-auto">
            <div className="max-h-[600px] overflow-y-auto space-y-2 pr-2 mb-4">
                {visibleChats.map((chat, index) => (
                    <ChatItem key={index} data={chat} onDelete={onDelete} />
                ))}
            </div>
